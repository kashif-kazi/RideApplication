...
import com.RideApp.strategies.RideStrategyManager;
...
//        TODO : Send notification to all the drivers about this ride request
	...
	public RideDto cancelRide(Long rideId) {
		Rider rider = getCurrentRider(); // get current driver

		Ride ride = rideService.getRideById(rideId);

		if (!rider.equals(ride.getRider())) {
			throw new RuntimeException(ride.getRider() + " not allowed to cancel the ride");
		}

		if (!ride.getRideStatus().equals(RideStatus.CONFIRMED)) {
			throw new RuntimeException(" can not end ride as Ride status is " + ride.getRideStatus());
		}
